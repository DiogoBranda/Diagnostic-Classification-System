...
while erros > 0 && numIteracao < maxNumIteracao
    for i=1 : trainSize
        ...
        % Erros
        ez= (trainDataNormalizado(i,3) - z(i)) * activationFunction_derivative(uk); % diferença entre o desejado e saida
...
for i=1:trainSize
  if trainData(i,3) == -1
    red  = [ red  ; trainDataNormalizado(i, 1:2) ];  % Adicionar valores da entrada quando o exemplo é da classe RED
  else
    blue = [ blue ; trainDataNormalizado(i, 1:2) ];  % Adicionar valores da entrada quando o exemplo é da classe BLUE
  end
end
for i=1:testSize
  if outputData(i,1) == -1
...
xlim([ inMin inMax ])
ylim([ inMin inMax ])
scatter( red(:,1) , red(:,2) , 25, 'red' , 'filled')% Pontos pertencentes à classe -1
scatter( blue(:,1), blue(:,2), 25, 'blue', 'filled')% Pontos pertencentes à classe +1
