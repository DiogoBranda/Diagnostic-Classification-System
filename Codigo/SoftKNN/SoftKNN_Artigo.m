...
Dados_Treino_Output_norm = normalizar_out(Treino(:,4));
TesteOutput= normalizar_out(TesteOutput);
%% Algoritmo
...
for i=1:tamanho_data_teste % Para cada ponto em que nao conheça o seu valor de output vai tentar descobir o seu valor atravez dos seu vizinhos
    ...
    for j=1:tamanho_data_treino % Hip^2=cat1^2+cat2^2
        distancia(1,j) = sqrt((TesteInput(i,1)- Treino(j,1))^2 + ...
        (TesteInput(i,2)- Treino(j,2))^2+(TesteInput(i,3)- Treino(j,3))^2);
    end
    ...
    for j=1:K % seleciona as K menores distancias
        ...
        if Dados_Treino_Output_norm(indexMinDist,1)== 0 %se a classe se for 1 
    ...
    for j=1:K %criar numerador e denuminador
        ...
        Dj=[Dj;gaussmf(0:0.0001:sqrt(3),[sigma menorDist(1,aux)])]; %Para termos tambem a ordem decrescente para fazer a matriz auxiliar alpha
        aux=aux-1;
    ...
    for j=1:K
        for l=1:K
            alpha(j,l)=max(min(Di(j,:),Dj(l,:))); % fizemos S como sendo max min.
                                                  %Com o min iremos criar um 
                                                  %array em que em cada posicao
                                                  % sera o valor minimo
                                                  % entre di e dj, depois
                                                  % disso escolhemos o
                                                  % maior valor desses
        end
    end
